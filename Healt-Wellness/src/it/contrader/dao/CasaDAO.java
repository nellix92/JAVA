...
public class CasaDAO {
    ...
    private final String QUERY_CREATE = "INSERT INTO casa (nome, prezzo, indirizzo, metri_quadrati, costruita, dettagli, tipo,  id_agenzia) VALUES (?,?,?,?,?,?,?,?)";

    private final String QUERY_UPDATE = "UPDATE casa SET nome=?, prezzo=?, indirizzo=?, metri_quadrati=?, costruita=?, dettagli=?, tipo=? WHERE id=?";
    ...
    private final String QUERY_FILTRA_PREZZO = "SELECT * FROM casa WHERE prezzo BETWEEN ? and ? ORDER BY prezzo ASC" ;
    private final String QUERY_FILTRA_METRI_QUADRATI = "SELECT * FROM casa WHERE CAST(metri_quadrati AS SIGNED) BETWEEN ? and ? ORDER BY metri_quadrati ASC" ;
    ...
    private final String QUERY_FILTRA_ANNO = "SELECT * FROM casa WHERE anno >= ? ";
    ...
    public List<Casa> getAll(){
        ...
        try{
            ...
            while(resultSet.next()){
                ...
                int agenzia_id = resultSet.getInt("id_agenzia");
                casa = new Casa(nome, prezzo, indirizzo, metri_quadrati, costruita, dettagli, tipo, agenzia_id);
                casa.setId(id);
                casaList.add(casa);
            }
        ...
        try {
            ...
            try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
                ...
                while (resultSet.next()) {
                    ...
                    Casa casa = new Casa(nome, prezzo, indirizzo, metri_quadrati, costruita, dettagli, tipo, agenzia_id);
                    casa.setId(id);
                    casaList.add(casa);
                }
            }
    ...
    public List<Casa> getFilterPrezzo(double minPrezzo, double maxPrezzo){
        List<Casa> caseInRange = new ArrayList<>();
        Connection connection = ConnectionSingleton.getInstance();
        try {
            String query = QUERY_FILTRA_PREZZO;
            try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
                preparedStatement.setDouble(1, minPrezzo);
                preparedStatement.setDouble(2, maxPrezzo);
    ...
    public List<Casa> getFiltroMetri(int minMetri, int maxMetri){
        List<Casa> caseInRange = new ArrayList<>();
        Connection connection = ConnectionSingleton.getInstance();
        try {
            String query = QUERY_FILTRA_METRI_QUADRATI;
            try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
                preparedStatement.setInt(1, minMetri);
                preparedStatement.setInt(2, maxMetri);
    ...
    public Casa read(int id){
        ...
        try{
            ...
            agenzia_id = resultSet.getInt("id_agenzia");
    ...
    public Casa findByFk(int fk){
        ...
        try{
            ...
            Casa casa = new Casa(id, nome, prezzo, indirizzo, metri_quadrati, costruita, dettagli, tipo, agenzia_id);
            casa.setAgenzia_id(resultSet.getInt("id_agenzia"));
    ...
    public boolean insert(Casa casaToInsert){
        ...
        try{
            ...
            preparedStatement.setInt(8, casaToInsert.getAgenzia_id());
    ...
    public boolean update(Casa casaToUpdate){
        ...
        if(!casaRead.equals(casaToUpdate)){
            try{
                ...
                preparedStatement.setInt(8, casaToUpdate.getId());
